...
for db in $DATABASES; do
  ...
  docker exec -t $CONTAINER_NAME pg_dump -U postgres $db | zstd -T0 -19 -o $BACKUP_DIR/$BACKUP_FILE
  ...
  rclone copy $BACKUP_DIR/$BACKUP_FILE r2:$BUCKET_NAME/backups/$TIMESTAMP
