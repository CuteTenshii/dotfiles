...
tar --exclude="$DATA_DIR/icon_cache" --exclude="$DATA_DIR/tmp" -czvf $BACKUP_DIR/vaultwarden-data-$TIMESTAMP.tar.gz $DATA_DIR/

echo "Uploading backups to R2 bucket..."
rclone copy $BACKUP_DIR/vaultwarden-data-$TIMESTAMP.tar.gz r2:$BUCKET_NAME/backups/$TIMESTAMP
